...
<body class="bg-green-50 font-sans">
  ...
  <section class="bg-green-200 py-10 text-center">
    <div class="max-w-4xl mx-auto px-4">
      ...
      <a href="#produk" class="bg-green-600 text-white px-6 py-2 rounded-full hover:bg-green-700">Jelajahi Produk</a>
    </div>
  </section>
    ...
    <div class="max-w-7xl mx-auto px-4">
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 justify-items-center">
        @forelse ($barangTitipan as $barang)
          <div class="bg-green-100 p-4 rounded-lg shadow hover:shadow-md transition w-full max-w-sm">
            ...
            <p class="text-gray-600 text-sm mt-1">{{ \Illuminate\Support\Str::limit($barang->deskripsi_barang, 50) }}</p>
            <p class="text-green-700 font-bold mt-2">Rp {{ number_format($barang->harga_barang, 0, ',', '.') }}</p>

            <!-- Status Garansi -->
            <p class="text-gray-600 text-sm mt-1">
              Garansi : {{$barang->garansi_barang}}
            </p>
      ...
      <!-- Form Cek Garansi -->
      <section class="bg-green-100 py-6 px-4 max-w-md mx-auto my-8 rounded shadow">
        <form action="{{ route('cek.garansi') }}" method="GET" class="flex gap-2">
          <input
            type="text"
            name="keyword"
            placeholder="Masukkan nama atau ID barang"
            class="flex-grow rounded border border-gray-300 px-3 py-2 focus:outline-green-500"
            required
          />
  ...
  <footer class="bg-green-800 text-white text-center py-6 mt-10">
    <p>&copy; {{ now()->year }} ReuseMart. Jual Beli Barang Bekas Berkualitas.</p>
  </footer>
