  ...
  <div class="max-w-5xl mx-auto py-10 px-6">
    ...
    <div class="bg-white rounded-xl shadow-md p-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
        <p class="text-2xl font-semibold text-green-700">Harga: Rp {{ number_format($barang->harga_barang, 0, ',', '.') }}</p>
      ...
      <p class="text-sm text-gray-500 mb-6">Jenis: {{ $barang->jenis_barang }} | Berat: {{ $barang->berat_barang }} gr</p>

      <!-- Gambar Barang -->
      <h3 class="text-xl font-semibold text-gray-800 mt-6 mb-3">Gambar Barang:</h3>
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
        @forelse($barang->gambarBarang ?? [] as $gambar)
          <div class="bg-gray-100 rounded-lg overflow-hidden shadow hover:scale-105 transition-transform duration-200">
      ...
      <!-- Tombol kembali -->
      <div class="mt-8">
        <a href="{{ url('/') }}" class="bg-green-600 text-white px-5 py-2 rounded-lg hover:bg-green-700 transition duration-200">
          ← Kembali
        </a>
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-md p-6">
      ...
      <form action="{{ route('diskusi.store', $barang->id_barang) }}" method="POST" class="mb-8">
        ...
        <div class="mb-4">
          ...
          <input type="text" name="nama_pengirim" class="w-full border border-gray-300 px-4 py-2 rounded-lg focus:ring-2 focus:ring-blue-400" required>
        </div>
        <div class="mb-4">
          <label class="block font-semibold text-gray-700 mb-1">Pesan</label>
          <textarea name="isi_pesan" class="w-full border border-gray-300 px-4 py-2 rounded-lg focus:ring-2 focus:ring-blue-400" rows="4" required></textarea>
        </div>
      ...
      @forelse($diskusi as $pesan)
        <div class="mb-5 p-4 border rounded-lg bg-gray-50 shadow-sm">
          <div class="flex justify-between items-center mb-1">
            ...
            <span class="text-sm text-gray-400">{{ \Carbon\Carbon::parse($pesan->created_at)->diffForHumans() }}</span>
          </div>
